...
const Contest = () => {
    ...
    const correctTask = <Tooltip title="задача сдана"><CheckCircleOutlined style={{color: "green"}}/></Tooltip>;
    const invalidTask = <Tooltip title="задача не сдана"><CloseCircleOutlined style={{color: "red"}}/></Tooltip>;
    const items = [
    ...
    return (
        <>
            <h1>Контест - STR EASY</h1>
            <Menu items={items} mode="horizontal" defaultSelectedKeys={['submit']} style={{borderRadius: 10, marginBottom: 20}} onClick={(item) => {choosePage(setPageContent, item.key)}}/>
            {pageContent}
        </>
    )
